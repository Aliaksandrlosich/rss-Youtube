  ...
  createCard(cardsItem) {
    const card =
      ...
      <div class="link"><a href=${cardsItem.link} target="_blank"><p class="title"> ${cardsItem.title}</a></p>
        ...
        <div class="info"><p><i class="fas fa-user-circle"></i> ${cardsItem.author}</p> 
          <p><i class="fas fa-eye"></i> ${cardsItem.viewCount}</p> 
          <p><i class="fas fa-thumbs-up"></i> ${cardsItem.likeCount} 
          <i class="fas fa-thumbs-down"></i> ${cardsItem.dislikeCount}</p>
        </div>
        <p class="date"><i class="fas fa-calendar-alt"></i> ${cardsItem.publishedDate}</p> 
...
// CONCATENATED MODULE: ./src/container/cardsContainer.js





class cardsContainer_CardsContainer {
  ...
  setCardsPerContainer() {
    ...
    this.cardsPerContainer = Math.floor(containerWidth / 310);
  ...
  updatePagesArray() {
    this.pagesArray = chunk_default()(this.playlist, this.cardsPerContainer);
  }
...
// CONCATENATED MODULE: ./src/search/searchBar.js


class searchBar_SearchBar {
  constructor() {
    this.id = [];
    this.nextPage = "";
    ...
    this.playlist = [];
    this.RESULTS_NUM = 15;
    this.API_KEY = "AIzaSyDdHPq6p5QH_Rmpat3DFmRUSlXN4d261Xg";
  ...
  addEvent() {
    ...
    document.addEventListener("keypress", (event) => {
      if ((event.keyCode === Enter || event.which === Enter) && event.target.value) {
    ...
    fetch(`${this.INFO_REQUEST_TEMPLATE}&q=${this.keyword}&maxResults=${this.RESULTS_NUM}&key=${this.API_KEY}${this.nextPage ? `&pageToken=${this.nextPage}` : ''}`)
      .then((response) => {
        if (response.status !== 200) {
          alert("Something wrong, status code" + response.status);
        } else {
          ...
          fetch(`${this.STAT_REQUEST_TEMPLATE}&id=${this.id.join(",")}&key=${this.API_KEY}`)
            ...
            .then((response) => {
              ...
              if (this.cardsContainer) {
                ...
                this.cardsContainer = new cardsContainer_CardsContainer(this.playlist, this);
...
const searchBar = new searchBar_SearchBar();
searchBar.init();
